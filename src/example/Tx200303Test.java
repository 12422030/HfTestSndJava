package example;

import java.util.ArrayList;
import java.util.List;

import cfca.safeguard.api.mps.ClientEnvironment;
import cfca.safeguard.api.mps.SGBusiness;
import cfca.safeguard.api.mps.bean.tx.upstream.Tx200303;
import cfca.safeguard.tx.Attachment;

/**
 * 账户持卡主体查询
 */
public class Tx200303Test {
    public static void main(String[] args) throws Exception {
        ClientEnvironment.initClientEnvironment("config");
        SGBusiness sgBusiness = new SGBusiness();

        Tx200303 tx200303 = new Tx200303();
        String mode = "01";
        String to = "313163000004";
        String fromTGOrganizationId="";
        String transSerialNumber = "cfcapolice01_0000000000000100002018050587303412075999";
        
        tx200303.setTransSerialNumber(transSerialNumber);
        tx200303.setApplicationID("20030320180505200303006");
        tx200303.setCaseNumber("AJ142100201601010812");
        tx200303.setCaseType("01");
        tx200303.setEmergencyLevel("02");
        tx200303.setSubjectType("1");
        tx200303.setBankID("002");
        tx200303.setBankName("交通银行");
        tx200303.setAccountName("李婷");
        tx200303.setCardNumber("6217380000000531");
        tx200303.setReason("主体查询测试");
        tx200303.setRemark("主体查询测试");
        tx200303.setApplicationTime("20180423000000");
        tx200303.setApplicationOrgID("010000");
        tx200303.setApplicationOrgName("五局");
        tx200303.setOperatorIDType("01");
        tx200303.setOperatorIDNumber("222303197705328831");
        tx200303.setOperatorName("徐");
        tx200303.setOperatorPhoneNumber("01061957036");
        tx200303.setInvestigatorIDType("01");
        tx200303.setInvestigatorIDNumber("222303197705328831");
        tx200303.setInvestigatorName("徐");

        List<Attachment> attachmentList = new ArrayList<Attachment>();
        Attachment attachment = new Attachment();
        attachment.setContent("");
        attachment.setFilename("heh.jpg");
        attachmentList.add(attachment);
        tx200303.setAttachments(attachmentList);

      
        try {
            sgBusiness.writeUpstreamXml(tx200303,fromTGOrganizationId,mode, to);
            System.out.println("写入文件成功");
        } catch (Exception e) {
            System.out.println("写入文件失败");
            e.printStackTrace();
        }
    }

}
