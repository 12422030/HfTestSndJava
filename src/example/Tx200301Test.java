package example;

import java.util.ArrayList;
import java.util.List;

import cfca.safeguard.api.mps.ClientEnvironment;
import cfca.safeguard.api.mps.SGBusiness;
import cfca.safeguard.api.mps.bean.tx.upstream.Tx200301;
import cfca.safeguard.tx.Attachment;

/**
 * 账户交易明细查询
 */
public class Tx200301Test {
    public static void main(String[] args) throws Exception {
        ClientEnvironment.initClientEnvironment("config");
        SGBusiness sgBusiness = new SGBusiness();

        Tx200301 tx200301 = new Tx200301();
        String mode = "01";
        String to = "313234001089";
        String fromTGOrganizationId="";
        String transSerialNumber = "cfcapolice01_000000000000010000201906278730341207513f";
        
        tx200301.setTransSerialNumber(transSerialNumber);
        tx200301.setApplicationID("2003012018050320030113f");
        tx200301.setCaseNumber("AJ130100201601111113");
        tx200301.setCaseType("01");
        tx200301.setEmergencyLevel("02");
        tx200301.setSubjectType("1");
        tx200301.setBankID("001");
        tx200301.setBankName("朝阳银行");
        tx200301.setAccountName("");
        tx200301.setCardNumber("520010120110100000712");
        tx200301.setInquiryMode("03");
        tx200301.setInquiryPeriodStart("20210623080000");
        tx200301.setInquiryPeriodEnd("20210624230000");
        tx200301.setReason("明细查询测试3");
        tx200301.setRemark("明细查询测试3");
        tx200301.setApplicationTime("20160118000000");
        tx200301.setApplicationOrgID("010000");
        tx200301.setApplicationOrgName("五局");
        tx200301.setOperatorIDType("01");
        tx200301.setOperatorIDNumber("222303197705328831");
        tx200301.setOperatorName("徐");
        tx200301.setOperatorPhoneNumber("01061957036");
        tx200301.setInvestigatorIDType("01");
        tx200301.setInvestigatorIDNumber("222303197705328831");
        tx200301.setInvestigatorName("徐");
        List<Attachment> attachmentList = new ArrayList<Attachment>();
        Attachment attachment = new Attachment();
        attachment.setContent("");
        attachment.setFilename("heh.jpg");
        attachmentList.add(attachment);
        tx200301.setAttachments(attachmentList);
       
        
        try {
            sgBusiness.writeUpstreamXml(tx200301,fromTGOrganizationId,mode,to);
            System.out.println("写入文件成功");
        } catch (Exception e) {
            System.out.println("写入文件失败");
            e.printStackTrace();
        }
    }

}
