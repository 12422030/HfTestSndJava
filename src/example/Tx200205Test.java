package example;

import java.util.ArrayList;
import java.util.List;

import cfca.safeguard.api.mps.ClientEnvironment;
import cfca.safeguard.api.mps.SGBusiness;
import cfca.safeguard.api.mps.bean.tx.upstream.Tx200205;
import cfca.safeguard.tx.Attachment;

/**
 * 冻结延期
 */
public class Tx200205Test {
    public static void main(String[] args) throws Exception {
        ClientEnvironment.initClientEnvironment("config");
        SGBusiness sgBusiness = new SGBusiness();

        Tx200205 tx200205 = new Tx200205();
        String mode = "01";
        String to = "313163000004";
        String fromTGOrganizationId = "";
        String transSerialNumber = "cfcapolice01_0000000000000100002018050446303412111222";

        tx200205.setTransSerialNumber(transSerialNumber);
        tx200205.setApplicationID("20020120180504200201022");
        tx200205.setApplicationType("01");
        tx200205.setCaseNumber("AJ130100201601010843");
        tx200205.setCaseType("01");
        tx200205.setOriginalApplicationID("20020120180504200201011");
        tx200205.setEmergencyLevel("01");
        tx200205.setBankID("002");
        tx200205.setBankName("交通银行");
        tx200205.setAccountType("01");
        tx200205.setAccountName("李婷");
        tx200205.setAccountNumber("6217380000000531_1");
        tx200205.setIDType("01");
        tx200205.setIDNumber("140302198503193382");
        tx200205.setFreezeType("01");
        tx200205.setBalance("30000");
        tx200205.setCurrency("RMB");
        tx200205.setPostponeStartTime("20190222000000");
        tx200205.setExpireTime("20190522000000");
        tx200205.setExtendRemark("测试");
        tx200205.setApplicationTime("20190222000000");
        tx200205.setApplicationOrgID("010000");
        tx200205.setApplicationOrgName("五局");
        tx200205.setOperatorIDType("01");
        tx200205.setOperatorIDNumber("222303197705328831");
        tx200205.setOperatorName("徐");
        tx200205.setOperatorPhoneNumber("01061957036");
        tx200205.setInvestigatorIDType("01");
        tx200205.setInvestigatorIDNumber("222303197705328831");
        tx200205.setInvestigatorName("徐");

        List<Attachment> attachmentList = new ArrayList<Attachment>();
        Attachment attachment = new Attachment();
        attachment
                .setContent("");
        attachment.setFilename("attachment.jpg");
        attachmentList.add(attachment);
        tx200205.setAttachments(attachmentList);

        try {
            sgBusiness.writeUpstreamXml(tx200205, fromTGOrganizationId, mode, to);
            System.out.println("写入文件成功");
        } catch (Exception e) {
            System.out.println("写入文件失败");
            e.printStackTrace();
        }
    }

}
