package example;

import java.util.ArrayList;
import java.util.List;

import cfca.safeguard.api.mps.ClientEnvironment;
import cfca.safeguard.api.mps.SGBusiness;
import cfca.safeguard.api.mps.bean.tx.upstream.Tx200203;
import cfca.safeguard.tx.Attachment;

/**
 * 冻结解除
 */
public class Tx200203Test {
    public static void main(String[] args) throws Exception {
        ClientEnvironment.initClientEnvironment("config");
        SGBusiness sgBusiness = new SGBusiness();

        Tx200203 tx200203 = new Tx200203();
        String mode = "01";
        String to = "313163000004";
        String fromTGOrganizationId = "";
        String transSerialNumber = "cfcapolice01_0000000000000100002018050446303412111333";

        tx200203.setTransSerialNumber(transSerialNumber);
        tx200203.setApplicationID("20020120180504200201033");
        tx200203.setApplicationType("01");
        tx200203.setCaseNumber("AJ130100201601010843");
        tx200203.setCaseType("01");
        tx200203.setOriginalApplicationID("20020120180504200201022");
        tx200203.setEmergencyLevel("01");
        tx200203.setBankID("002");
        tx200203.setBankName("交通银行");
        tx200203.setAccountType("01");
        tx200203.setAccountName("李婷");
        tx200203.setAccountNumber("6217380000000531_1");
        tx200203.setIDType("01");
        tx200203.setIDNumber("140302198503193382");
        tx200203.setWithdrawalRemark("测试冻结解除");
        tx200203.setApplicationTime("20190222000000");
        tx200203.setApplicationOrgID("010000");
        tx200203.setApplicationOrgName("五局");
        tx200203.setOperatorIDType("01");
        tx200203.setOperatorIDNumber("222303197705328831");
        tx200203.setOperatorName("徐");
        tx200203.setOperatorPhoneNumber("01061957029");
        tx200203.setInvestigatorIDType("01");
        tx200203.setInvestigatorIDNumber("222303197705328831");
        tx200203.setInvestigatorName("徐");

        List<Attachment> attachmentList = new ArrayList<Attachment>();
        Attachment attachment = new Attachment();
        attachment
                .setContent("");
        attachment.setFilename("attachment.jpg");
        attachmentList.add(attachment);
        tx200203.setAttachments(attachmentList);

        try {
            sgBusiness.writeUpstreamXml(tx200203, fromTGOrganizationId, mode, to);
            System.out.println("写入文件成功");
        } catch (Exception e) {
            System.out.println("写入文件失败");
            e.printStackTrace();
        }
    }

}
